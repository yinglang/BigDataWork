...
typedef struct {
    double a = 2.0;
    char *s= NULL;
    map<int, double>* score=NULL;
}MsgType;

#define ValueType MsgType

class VERTEX_CLASS_NAME(InputFormatter): public InputFormatter{
public:

    int getVertexValueSize() override {
        m_n_value_size = sizeof(MsgType);
    ...
    void init_all_zeor_outdegree_vertex(int64_t start, int64_t end){
    ...
    void loadGraph(){
        ...
        //init_all_zeor_outdegree_vertex(0, from);
        ...
        for(int64_t i = 1; i < m_total_edge; i++){
            ...
            if(last_vertex != from){
                ValueType v;
                addVertex(last_vertex, &v, outdegree);
                //init_all_zeor_outdegree_vertex(last_vertex+1, from);
                last_vertex = from;
                outdegree = 1;
            }else{
                outdegree ++;
        ...
        ValueType v;
        addVertex(last_vertex, &v, outdegree);
        //init_all_zeor_outdegree_vertex(last_vertex+1, m_total_vertex);
    }
};
...
int VERTEX_CLASS_NAME(Graph)::a = 1;
map<string, int> VERTEX_CLASS_NAME(Graph)::vmap;

#define EPS 1e-6
// V value, E value, M value
class VERTEX_CLASS_NAME(): public Vertex<MsgType, double, MsgType>{
public:
    void init_zero_degree_value(){
        mutableValue()->s = new char[8];
        mutableValue()->a = 1.0;
    }
    ...
    void compute(MessageIterator *pmsgs) override {
        ...
        if(getSuperstep() == 0){
            if(mutableValue()->s == NULL)
                init_zero_degree_value();
        } else if(getSuperstep() == 1){
            cout <<"vid: " << getVertexId() << ";";
//            cout <<  "; a: " << VERTEX_CLASS_NAME(Graph)::a;
//            VERTEX_CLASS_NAME(Graph)::a ++;
//            VERTEX_CLASS_NAME(Graph)::vmap["aa"] = VERTEX_CLASS_NAME(Graph)::a;
//            cout << "; vmap['a']: " << VERTEX_CLASS_NAME(Graph)::vmap["aa"] << endl;
//            if(!pmsgs->done()) {
//                cout << "msg: " << pmsgs->getValue().a << ", " << (void*)(pmsgs->getValue().s.data()) << ";" << endl;
//            }
//            cout << getValue().s << endl;
            cout <<"value: " << (getValue().s) << ", " << getValue().a << "; out degree: " << getOutEdgeIterator().size() << endl;
        }
        ...
        if(getSuperstep() == 0){ // init
            ...
            for(; ! pmsgs->done(); pmsgs->next()){  // core cal
                sum += pmsgs->getValue().a;
            }


            val = 0.15 + 0.85 * sum;

            double acc = fabs(getValue().a - val);
            accumulateAggr(0, &acc);                        // id 0 aggregator
        }

        mutableValue()->a = val;
        mutableValue()->s[0] = 'h';        mutableValue()->s[1] = '\0';
