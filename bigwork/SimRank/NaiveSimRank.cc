...
#define VERTEX_CLASS_NAME(name) NaiveSimRank##name
#define EPS 1e-6
...
public:
    ...
    void init(int argc, char* argv[]) {
        ...
        if (argc < 6) {
           printf ("Usage: %s <input path> <output path> <iterator k> <max_pathLenth l> <arg c>\n", argv[0]);
           exit(1);
        }
...
//        cout.open("/home/hui/github/BigDataWork/bigwork/SimRank/log.txt", ios_base::app);
        ...
        if(getSuperstep() == K){
            ...
            for(uint64_t i = 0; i + 1 < msgs.size(); i++){
                //for(uint64_t j = i + 1; )
            }
        }

        // send it sim map to all out-degree vertex
        merge_msg_struct mmsg;
        mmsg.source_id = (uint64_t) getVertexId();
        mmsg.sim = *(getValue().sim);
        for(auto it=getOutEdgeIterator(); !it.done(); it.next()){
            send_merge_msg(it.target(), mmsg);
        }
    }
